...
export default function Dashboard() {
  ...
  const actions = [
    { name: 'Register New Reader', icon: UserPlus, path: '/readers', color: 'hover:border-blue-500/40 text-blue-400 hover:bg-blue-500/5' },
    { name: 'Add Book to Catalog', icon: PlusCircle, path: '/books', color: 'hover:border-emerald-500/40 text-emerald-400 hover:bg-emerald-500/5' },
    { name: 'Record Lending Transaction', icon: Bookmark, path: '/lending', color: 'hover:border-amber-500/40 text-amber-400 hover:bg-amber-500/5' },
  ];
  ...
  return (
    <div className="flex-1 p-6 md:p-8 space-y-8 bg-[#020617] overflow-y-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-800/60 pb-6">
        ...
        <div className="flex items-center gap-2 bg-slate-900/60 border border-slate-800 px-4 py-2 rounded-xl text-xs font-semibold text-slate-400">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div 
            key={stat.name} 
            className="glass-panel rounded-xl p-5 border shadow-lg flex flex-col justify-between hover:scale-[1.02] transition-transform duration-300"
          >
            <div className="flex justify-between items-start">
              <div className="space-y-1">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">{stat.name}</span>
                {stat.loading ? (
              ...
              <div className={`p-3 rounded-lg bg-gradient-to-tr border ${stat.color}`}>
                <stat.icon className="w-6 h-6" />
              </div>
            </div>
            <p className="text-xs text-slate-500 font-medium mt-4 pt-4 border-t border-slate-800/40">
              {stat.description}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
            ...
            <div className="flex flex-col gap-3">
              {actions.map((act) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between border border-slate-800/80 bg-slate-900/30 rounded-xl p-4 text-xs font-bold transition-all duration-300 ${act.color}`}
                >
          ...
          {/* System Information */}
          <div className="glass-panel rounded-2xl p-6 shadow-xl space-y-4">
            <h3 className="text-md font-bold text-white mb-2">System Profile</h3>
            <div className="text-xs space-y-3.5">
              ...
              <div className="flex justify-between border-b border-slate-800/40 pb-2">
                ...
                <span className="text-teal-400 font-bold">MongoDB Local</span>
              </div>
              <div className="flex justify-between border-b border-slate-800/40 pb-2">
                <span className="text-slate-400">JWT Authentication</span>
                <span className="text-slate-200 font-bold">Enabled (Bearer)</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Mailer Service</span>
                <span className="text-amber-400 font-bold">Mailtrap Host</span>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-2">
          <div className="glass-panel rounded-2xl p-6 shadow-xl h-full flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-800/40">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-md font-bold text-white">Recent System Activities</h3>
                </div>
                <span className="text-[10px] uppercase font-bold tracking-wider bg-slate-800 text-slate-400 px-2.5 py-1 rounded-full">
                  Audit Feed
                </span>
              </div>
              ...
              <div className="divide-y divide-slate-800/40">
                {activities.map((act) => (
                  <div key={act.id} className="py-3.5 flex items-start justify-between gap-4">
                    ...
                    <span className="text-[9px] font-bold px-2 py-0.5 rounded bg-slate-900 border border-slate-800 text-slate-400 uppercase tracking-widest shrink-0">
            ...
            <p className="text-center text-[11px] text-slate-500 mt-6 pt-4 border-t border-slate-800/40 font-medium">
